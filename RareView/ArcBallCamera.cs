...
namespace RareView
{
	...
	public class ArcBallCamera
	{
		...
		*/
		...
		public void MoveAxial(Vector3 Movement)
		{
			Target.X += Movement.X;
			Target.Y += Movement.Y;
			Target.Z += Movement.Z;
		...
		public void MoveDirectional(Vector3 velocity)
		{
			var curMtx = GetMatrix();
			var forward = new Vector3(curMtx[0, 2], curMtx[1,2], curMtx[2,2]);
			var up = new Vector3(curMtx[0,1], curMtx[1,1], curMtx[2,1]);
			var right = Vector3.Cross(forward, up).Normalized();
			
			var movement = new Vector3(0.0f, 0.0f, 0.0f);
			
			movement += (forward * velocity.Z);
			movement += (right * velocity.X);
